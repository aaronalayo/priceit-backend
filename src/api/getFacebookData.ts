...
function modifyFacebookRequestBody(searchWord:string, page:number) {
  const body: string =
...
export const getFacebookData = async (searchWord:string, page:number) => {
  ...
  try {
    ...
    if (data.data.marketplace_search.feed_units.page_info.has_next_page === true) {
      let endCursor = JSON.parse(JSON.parse(JSON.stringify(data.data.marketplace_search.feed_units.page_info.end_cursor)));
      // console.log('facebook.ts.getFacebookData: endCursor', endCursor);
    }
